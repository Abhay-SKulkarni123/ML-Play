...
async def train(session_id: uuid.UUID, body: TrainRequest, db: AsyncSession = Depends(get_db)):
    ...
    if len(df) > MAX_DATASET_ROWS:
        raise HTTPException(status_code=400,
            detail=f"Dataset has {len(df)} rows. Max allowed: {MAX_DATASET_ROWS}.")
    ...
    if session.task_type == "classification":
        ...
        if len(class_counts) < 2:
            raise HTTPException(status_code=400,
                detail="Classification requires at least 2 classes.")
    ...
    try:
        ...
        raise HTTPException(status_code=408,
            detail=f"Training timed out after {MAX_TRAINING_SECONDS}s.")
...
async def tune(session_id: uuid.UUID, body: TrainRequest, db: AsyncSession = Depends(get_db)):
    ...
    def get_model(trial, name: str):
        ...
        if name == "random_forest":
            ...
            return XGBClassifier(**p, eval_metric="logloss") if task == "classification" else XGBRegressor(**p)
            ...
            return DecisionTreeClassifier(**p) if task == "classification" else DecisionTreeRegressor(**p)
            ...
            return KNeighborsClassifier(**p) if task == "classification" else KNeighborsRegressor(**p)
            ...
            return GradientBoostingClassifier(**p) if task == "classification" else GradientBoostingRegressor(**p)
...
async def predict(session_id: uuid.UUID, body: PredictRequest, db: AsyncSession = Depends(get_db)):
    ...
    for col in X_train.columns:
        if col not in input_df.columns:
            input_df[col] = float(X_train[col].median())
